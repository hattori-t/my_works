...
dir.create(paste("Prediction_type2_",data1,"_to_",data2,sep=""))
...
Prediction.rrBLUP <- function(Method){
  ...
  for(i in 1:nrow(test)){
    ...
    for(k in 1:ncol(test)){
      ...
      Result <- kinship.BLUP(y = training[,k], G.train = Geno[!(rownames(Pheno) %in% removes),], G.pred = Geno_test[F1name,,drop = FALSE], K.method = Method)
...
for(i in 1:ncol(test)){
  pdf(paste("Prediction_type2_",data1,"_to_",data2,"/",phenolist[i],"_rrBLUP.pdf",sep=""))
...
write.csv(Predictedvalues.RR, paste("Prediction_type2_",data1,"_to_",data2,"/Predictedvalues_rrBLUP.csv",sep=""))
rownames(cor_rrBLUP) <- colnames(test)
write.csv(cor_rrBLUP, paste("Prediction_type2_",data1,"_to_",data2,"/cor_rrBLUP.csv",sep=""))
rownames(rmse_rrBLUP) <- colnames(test)
write.csv(rmse_rrBLUP, paste("Prediction_type2_",data1,"_to_",data2,"/rmse_rrBLUP.csv",sep=""))
...
for(i in 1:ncol(test)){
  pdf(paste("Prediction_type2_",data1,"_to_",data2,"_RKHS/",phenolist[i],"_RKHS.pdf",sep=""))
...
write.csv(Predictedvalues.RKHS, paste("Prediction_type2_",data1,"_to_",data2,"_RKHS/Predictedvalues_RKHS.csv",sep=""))
