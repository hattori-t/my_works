...
####### LOO for 3 training data <rrBLUP> : inbred, F1-x, inbred+F1-x (different from testdata)
...
dir.create(paste("rrBLUP_LOO_",data,"_",trainingdata,"_to_",testdata, sep = ""))


## G-BLUP ##
regression <- "G-BLUP"
dir.create(paste("rrBLUP_LOO_",data,"_",trainingdata,"_to_",testdata,"/",regression, sep = ""))
...
for(k in 1:ncol(pheno)){
  ...
  for(i in 1:nrow(test)) {
    ...
    x <- amat[xname,xname]
    input <- data.frame(y=y.train,ID=xname)
    res <- kin.blup(input, geno="ID", pheno="y", K=x)
    y.pred[i] <- tail(res$pred,1)
  }

  prediction[,k] <- as.vector(y.pred)

  #plot
  pdf(paste("rrBLUP_LOO_",data,"_",trainingdata,"_to_",testdata,"/",regression,"/",traitname,".pdf",sep = ""))
...
write.csv(prediction, paste("rrBLUP_LOO_",data,"_",trainingdata,"_to_",testdata,"/",regression,"/predictedvalues_",regression,".csv",sep = ""))
write.csv(result, paste("rrBLUP_LOO_",data,"_",trainingdata,"_to_",testdata,"/",regression,"/result_",regression,".csv",sep = ""))
