...
dir.create(paste("GS_LOO_",data,"_inbred", sep = ""))


#### GS rrBLUP #######################################################
require(rrBLUP)
regression <- "G-BLUP"
dir.create(paste("GS_LOO_",data,"_inbred/",regression, sep = ""))

prediction <- matrix(NA, nrow=nrow(pheno),ncol=ncol(pheno), dimnames = dimnames(pheno))
...
for(k in 1:ncol(pheno)){
  ...
  for(i in 1:length(y)) {
    ...
    y.pred[i] <- res$g.pred + rep(res$beta, length(res$g.pred))
  }

  prediction[selector,k] <- as.vector(y.pred)
  
  #plot
  pdf(paste("GS_LOO_",data,"_inbred/",regression,"/",traitname,".pdf",sep = ""))
  plot(pheno[,k], prediction[,k], xlab = "Observed Value", ylab = "Predicted Value", main = paste(colnames(pheno)[k],"_",regression,sep = ""))
  abline(0, 1, lty = "dotted")
  cor <- cor(pheno[,k], prediction[,k], use="pair")
  mse <- sum((pheno[,k] - prediction[,k])^2,na.rm = T) / length(pheno[,k])
  rmse <- sqrt(mse)
...
write.csv(prediction, paste("GS_LOO_",data,"_inbred/",regression,"/predictedvalues_",regression,".csv",sep = ""))
write.csv(result, paste("GS_LOO_",data,"_inbred/",regression,"/result_",regression,".csv",sep = ""))
