setwd("C:/Users/Tomo/Desktop/sorghum_local/GS_2014_Mex")
...
CreateRandomPartition<-function(N,Nfold,Nrepeat){
    ...
    for(r in 1:Nrepeat){
        ...
        print(write(c(Nfold,ceiling(N/Nfold)),Output,nc=2))
        Partition<-c(Partition,rep(-9,Nfold*ceiling(N/Nfold)-N))
        write(matrix(Partition,nc=ceiling(N/Nfold),nr=Nfold),Output,nc=Nfold,append=TRUE)
    }
}
...
write.csv(cor.vec, "Core_151021.csv", quote = F)

require(gplots)

pdf("Core_151021.pdf")
heatmap.2(
as.matrix(cor.vec),                     # 行�?�型�?ータ
main = "2013_Mex�?,     # グラフタイトル
Rowv = TRUE,              # 列にクラスタ図を描�?
Colv = TRUE,                # 行にクラスタ図を描�?
